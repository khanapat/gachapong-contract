...
describe("Jackpot", function () {
  ...
  async function approveToken(
    ...
    expect(await token.allowance(acc.address, jackpot.address)).to.equal(
      amount
    );
  }

  before(async function () {
    [owner, worker, wallet, gachapong, addr1, addr2, addr3, ...addrs] =
  ...
  beforeEach(async function () {
    token = (await Token.deploy("USDC TEST", "USDC")) as StableCoin;
    ...
    // wallet
    await expect(() =>
      token.connect(owner).transfer(hotWallet.address, eth(100000))
    ).to.changeTokenBalance(token, hotWallet, eth(100000));

    await hotWallet.connect(owner).approveToken(token.address, jackpot.address);
  ...
  describe("SetWallet", function () {
    ...
    it("Should be unable to set wallet because of address 0", async function () {
      await expect(
        jackpot.connect(owner).setWallet(ethers.constants.AddressZero)
  ...
  describe("AddTicket", function () {
    it("Should be able to add ticket with equal ticket price", async function () {
      ...
      expect(
        (await jackpot.getTickets(addr1.address, jackpotRound0)).length
      ).to.equal(1);
    });
    ...
    it("Should be able to add ticket with 1 ticket price 2 times", async function () {
      ...
      expect(
        (await jackpot.getTickets(addr2.address, jackpotRound0)).length
      ).to.equal(0);
    });

    it("Should be unable to add ticket because of not gachapong role", async function () {
      await expect(jackpot.connect(addr1).addTicket(addr1.address, eth(100))).to
        .be.reverted;
    });
  });
  ...
  describe("ClosePool", function () {
    ...
    it("Should be unable to close pool because of ref", async function () {
      ...
      const currentBlockNumber = await ethers.provider.getBlockNumber();
      await expect(
        jackpot.connect(worker).closePool(currentBlockNumber)
  ...
  describe("GenerateRandom", function () {
    it("Should be able to generate random", async function () {
      ...
      await expect(
        jackpot.connect(worker).generateRandom(jackpotRound0)
      ...
      expect(
        await jackpot.ticketOwner(jackpotRound0, result.winnerId)
  ...
  describe("ClaimReward", function () {
    it("Should be able to claim reward", async function () {
      ...
      expect(await jackpot.viewReward(jackpotRound0, addr1.address)).to.equal(
        eth(120 * 0.3)
      );
      ...
      expect(await token.balanceOf(addr1.address)).to.equal(
        eth(1000).add(eth(120 * 0.3))
      );
      expect(await token.balanceOf(hotWallet.address)).to.equal(
        eth(100000).sub(eth(120 * 0.3))
      );
    });

    it("Should be unable to claim reward because of not this time", async function () {
      await expect(
        jackpot.connect(addr1).claimReward(jackpotRound0)
    ...
    it("Should be unable to claim reward because of not winner", async function () {
      ...
      await expect(
        jackpot.connect(addr2).claimReward(jackpotRound0)
